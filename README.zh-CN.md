...
> 📢 **更新日志** —— 完整发布说明见 [Releases 页面 →](https://github.com/larryxiao/openquack/releases)
>
> - 🎛️ **[v2.0.0-alpha.22](https://github.com/larryxiao/openquack/releases/tag/v2.0.0-alpha.22) —— 让 AI 润色按你的想法来。** 本地润色的系统提示词现在可以在"设置"里直接编辑(带一键恢复默认),想让它更好地处理中英混排空格,或修正听写误判,自己写规则就行。录音浮层现在会跟到你当前所在的 Space,保存的历史记录条数上限也从启动那一刻就生效了。
> - ✨ **[v2.0.0-alpha.21](https://github.com/larryxiao/openquack/releases/tag/v2.0.0-alpha.21) —— 转写更快,AI 润色可选开启。** ANE 性能问题修复(转写速度回归解决),首次设置流程现在会引导你开启本地 LLM 润色,不用自己去翻设置。
> - 🛡️ **[v2.0.0-alpha.20](https://github.com/larryxiao/openquack/releases/tag/v2.0.0-alpha.20) —— 不会再卡死了。** 录音过程中麦克风变化(比如 AirPods 连接、切换设备),OpenQuack 现在会干净地停止并保留你已经说的内容,而不是卡住。另外还有本地转写润色、应用内模型切换、更快的首次启动。
...
- **本地 LLM 润色(实验性,默认关闭)。** 用一个本地小模型再过一遍你的转写,收紧措辞、修正明显的听写错误 —— 全程本地,不联网。默认关闭;可以在首次设置或"设置"里开启,提示词也能自己改。
...
- **上下文感知转写** —— OpenQuack 会读取你即将粘贴位置周围的文本 —— 光标上方那行、你所在的函数、你正在回复的聊天串 —— 把它作为上下文喂给语音模型。专业术语会根据你正在做的事情自动消歧(在终端里, "cloud code" 会被识别成 "Claude Code",而不是反过来)。

功能还在排队,等适配基础打牢再上。想看具体排期见 [`docs/ROADMAP.md`](docs/ROADMAP.md);整体方向见 [`docs/VISION.md`](docs/VISION.md)。

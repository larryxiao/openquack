...
public enum Report {
    public static func writeMarkdown(_ results: [BenchResult], host: HostInfo, to url: URL) throws {
        ...
        md += "| Engine | Model | WER | CER | RTF | Wall (avg) | Cold start | Peak RSS | Clips |\n"
        md += "|---|---|---:|---:|---:|---:|---:|---:|---:|\n"
        ...
        for r in sorted {
            ...
            md += "| Clip | Audio | Wall | RTF | WER | CER | Lang | Preview |\n"
            md += "|---|---:|---:|---:|---:|---:|---|---|\n"
            for c in r.perClip {
    ...
    public static func writeCSV(_ results: [BenchResult], to url: URL) throws {
        var csv = "engine,model,clip_id,audio_seconds,wall_seconds,rtf,wer,cer,ttft_seconds,detected_language,text_preview\n"
        for r in results {
            for c in r.perClip {
                ...
                csv += "\(r.engineName),\(r.modelID),\(c.clipID),\(c.audioSeconds),\(c.wallSeconds),\(rtfVal),\(c.wer),\(c.cer),\(ttftStr),\(c.detectedLanguage ?? ""),\(preview)\n"
            }
        }
